    ...
    def forward(self, x):
        ...
        for end_idx in idx_crops:
            ...
            if isinstance(_out, tuple):
                _out = _out[0]
            _out = torch.mean(_out, dim=-1)
            output = torch.cat((output, _out))
            start_idx = end_idx
        n_batches_views, n_vars, d_model = output.shape
        #n_batches_views = output.shape[0]
        output = output.reshape(n_batches_views, -1)
        #output = output.reshape(n_batches_views * n_vars, d_model)
